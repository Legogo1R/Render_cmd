...
def scripts2string(scrits_before, scripts_after, scripts_optional):
    ...
    str_scripts_before = " -P ".join(filter(None, tmp_lst))  # Needs to be as 1 string
    
    tmp_lst = []
    for script_name in scripts_after:
        if script_name != '': 
            tmp_lst.append(os.path.sep.join([MAIN_PATH, SCRIPT_PATH, PERM_SCRIPTS_PATH, script_name]))
    str_scripts_after = " -P ".join(filter(None, tmp_lst))  # Needs to be as 1 string
    ...
    str_optional_scripts = " -P ".join(filter(None, tmp_lst))  # Needs to be as 1 string

    # Concatenate in a single string
    arg_scripts = " -P ".join(filter(None, (str_scripts_before,str_optional_scripts,str_scripts_after)))
    return f'-P {arg_scripts}'
